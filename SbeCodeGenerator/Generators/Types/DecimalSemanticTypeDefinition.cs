...
namespace SbeSourceGenerator
{
    internal record DecimalSemanticTypeDefinition(string Namespace, string Name, List<IFileContentGenerator> Fields) : IFileContentGenerator
    {
        public void AppendFileContent(StringBuilder sb, int tabs = 0)
        {
            ...
            sb.AppendLine("{", tabs);
            sb.AppendSummary("Decimal value from mantissa and exponent", tabs + 1, nameof(DecimalSemanticTypeDefinition));
            sb.AppendLine($"public decimal{(hasNullable ? "?" : "")} Value => Mantissa.ToDecimalWithPrecision(-Exponent);", tabs + 1);
            sb.AppendLine("}", tabs);
        }
    }
}